...
** Have dirs named shared/tier1, shared/tier2, etc that contain only unexported symbols that packages in that tier
   can access via a dot import
** dirs in tier1 are compiled separately, with each dir having access only to itself and shared/tier1
** dirs in tier2 are compiled separately, with each dir having access only to itself, shared/tier2, and all tier1 dirs

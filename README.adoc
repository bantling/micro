...
** default numeric type is *big.Rat, but custom conversion functions can be used for some other numeric type
* util
** Provides ErrorReader to return a specified error after reading a specified set of bytes; useful for unit tests.

== TODO

* conv.IntToFloat function that considers the case of ints outside the range of mantissa bits, but are a power of 2
  multiple of a value that is in the range of mantissa, such that an non-zero exponent could be used.
  Verify such ints can just be cast to floats with go syntax, and represented properly.
* Math on plain strings that are decimal values - digits possibly followed by a dot and more digits - 4 basic ops

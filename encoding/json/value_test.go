...
func TestToValue_(t *testing.T) {
	...
	assert.Equal(t, union.OfError[Value](fmt.Errorf("AnyTo cannot convert the input type chan bool")), union.OfResultError(ToValue(map[string]any{"foo": c})))

	// Slice failure
	assert.Equal(t, union.OfError[Value](fmt.Errorf("AnyTo cannot convert the input type chan bool")), union.OfResultError(ToValue([]any{c})))
}

...
func TestInit_(t *testing.T) {
  ...
  assert.Equal(t, map[string]Value{"foo": Value{typ: Number, val: union.Of4V[map[string]Value, []Value, string, bool]("0")}}, mv)
  ...
  sv := []Value{}
	...
	assert.Equal(t, []Value{Value{typ: Number, val: union.Of4V[map[string]Value, []Value, string, bool]("0")}}, sv)

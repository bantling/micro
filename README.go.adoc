...
* rewrite json, decimal, and range to register conversion funcs with conv, so that conv.To can convert to/from those types
** json, decimal, and range can use conv.to internally to perform the function
* update this README
* simple ORM
** map structs, assuming each field camelcase name translates to snake case column name

...
* Create a BASIC language and code generator to convert it into Go/SQL code
** funcs, sql style case statements, vars of int (int64), uint (uint64), string, double, arrays, maps, structs
** stream-style ability to easily process JSON (pick it apart and reconstruct) - eg, pick apart json object as 2d array,
   mess with inner array like rows, combine rows back into an object
** send events over the wall
*** allows easily changing code and deployment organization eg, monorepo to multiple repos, one docker to many dockers
** validate values
** native sql support for vanilla insert, update, delete, select, upsert, views
*** common types - string, fulltext, int, int64, decimal
*** read sql standards for types
*** allow mixing vendor specific sql with standard sql
*** generate DDL automatically from struct definitions for each supported database
*** start with support for postgres, mysql, mariadb, oracle, tsql, h2, hsql, sqlite
** Just use plain old Go text template
** For specific purposes, like HTTP handlers and SQL DTO/DAO structs
** Generate code for DTOs, DAOs, REST handlers, HTML
** Define data types
...
*** Use a set of pre-defined types and adapt to/from actual types supported by Go/databases
...
** generate queries that use decimal type for accurate math calculations

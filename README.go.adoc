...
* conv complete testing
** conv and WrapperInfo needs to be revamped somehow - conv.LookupConversion requires args of reflect.Type to determine
   if a conversion is possible, but WrapperInfo requires instances of reflect.Value and reflect.Type
** Add a capability for json.NullValue to be equivalent to a nil pointer or empty Maybe by LookupConversion
* alter Makefile to drop docker support, only have podman

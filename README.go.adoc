...
* make json Value better
** search by path to get/set a value
** path can dig into map keys and array indexes
** simple path using . for keys, and [] for indexes
** eg .addresses[3].city => top object key addresses is an array, index 3 is an object, key city is read/written
* modify events to be less simplistic
** ability to modify chain of handlers on the fly in a self-organizing way
** when new handlers added, can specify they come before another event, after another event, or both

...
* conv
** struct serialization/deserialization ideas
*** Allow sum types for deserializing (eg, a discriminator field indicates which struct type to use)
*** Serialize only non-zero value fields
*** Flattened structure, where child struct fields are considered parent struct fields
*** Deserialize any unknkown keys into catch all field
*** Choose encoding for []byte fields (eg base64)
*** JSON Lines (each line is one object)
*** Patch documents (see https://datatracker.ietf.org/doc/html/rfc6902)
